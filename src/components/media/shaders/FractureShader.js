...
const fractureFragmentShader = `
    ...
    vec2 get_img_uv() {
        ...
        // Simplified mapping for our canvas use case
        return vec2(img_uv.x, 1.0 - img_uv.y);
    }
    ...
    void main() {
        ...
        vec2 pointer_direction = normalize(u_pointer_position - uv);
        ...
        if (u_effect_active > 0.) {
            img_uv = get_disturbed_uv(img_uv, sector_constant, cracks_edge, pointer_direction, 0.0);
        }
        ...
        // Draw cracks in white/light
        if (u_effect_active > 0.) {
            float crack_vis = max(is_grid_edge, is_central_edge);
            crack_vis = max(crack_vis, central_cracks);
            if (crack_vis > 0.1) {
                // color = mix(color, vec3(1.), crack_vis * 0.5);
                // Simple lighten for cracks
                color += vec3(0.3) * crack_vis;
            }
        }
